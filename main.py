...
SERVER = "192.168.0.181" # what sever to start the connection on 
ADDR = (SERVER, PORT)
...
def handle_client(conn, addr):
    ...
    while connected:
        ...
        if msg_length:
            ...
            msg = conn.recv(msg_length).decode(FORMAT) # than receives the message 
            if msg == DISCONNECT_MESSAGE:
                connected = False
            elif msg == "U": # if the client sends an Upper case U it sends the last thing sent to the server
...
def start(): # function to start the server 
    ...
    while True:
        ...
        thread = threading.Thread(target=handle_client, args=(conn, addr))# starts a function for the new client on a thread
        thread.start()
        print(f"[ACTIVE CONNECTIONS] {threading.activeCount() - 1}") # tells how many computers are connected to the server 
